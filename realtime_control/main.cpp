...
int main(VOID)
{
    ...
    unsigned short IOSendType = 0; // 送信入出力信号データ指定
    unsigned short IORecvType = 0; // 返信入出力信号データ指定
    unsigned short IOBitTop = 0;
    unsigned short IOBitMask = 0xffff;
    unsigned short IOBitData = 0;

    cout << "Input connection destination IP address (172.30.6.121) -> ";
    cin.getline(dst_ip_address, MAXBUFLEN);
    if (dst_ip_address[0] == 0)
        strcpy(dst_ip_address, "172.30.6.121");
    ...
    {
        cout << "What is target? input/output signal([I]nput / [O]utput)-> ";
        cin.getline(msg, MAXBUFLEN);
        switch (msg[0])
        {
        case 'O': // 対象を出力信号にする
    ...
    cout << "[ 0:None / 1:XYX / 2:JOINT / 3:PULSE ]\n";
    cout << "--- Please input the number --- [0] to [3] ->";
    cin.getline(msg, MAXBUFLEN);
    type = atoi(msg);
    for (int k = 0; k < 4; k++)
    {
        ...
        cout << "--- Please input the number --- [0] to [11] ->";
        cin.getline(msg, MAXBUFLEN);
        type_mon[k] = atoi(msg);
    }
    sprintf(msg, "IP=%s / PORT=%d / Send Type=%d / Monitor Type 0/1/2/3=%d/%d/%d/%d", dst_ip_address, port, type,
    ...
    long ratio = 1;
    int retry;
    fd_set SockSet;   // select で使用するソケット集合体
    timeval sTimeOut; // タイムアウト設定用
    ...
    while (loop)
    {
        ...
        // キーボード入力
        // [Enter]=終了 ／ [d]=モニタデータ表示 ／ [z/x]=送信する指令データの１つめをdelta 分だけ増減
        while (_kbhit() != 0)
        {
            ch = _getch();
            switch (ch)
        ...
        while (retry)
        {
            FD_ZERO(&SockSet);            // SockSet 初期化
            FD_SET(destSocket, &SockSet); // ソケットを登録
            sTimeOut.tv_sec = 1;          // 送信タイムアウト設定 (sec)
            sTimeOut.tv_usec = 0;         // (μsec)
                ...
                case MXT_TYP_FB_JOINT:
                    ...
                    if (disp)
                    {
                        ...
                        sprintf(buf, "Receive(%ld): TCount=%d Type(JOINT)=%d\n %7.2f, %7.2f, %7.2f, %7.2f, %7.2f, %7.2f, %7.2f, %7.2f(%s)"
                                , MXTrecv.CCount, MXTrecv.TCount, DispData, j->j1, j->j2, j->j3, j->j4, j->j5, j->j6, j->j7, j->j8, str);
                        cout  << buf << endl;
                    }
                    break;
                case MXT_TYP_POSE:
                ...
                case MXT_TYP_FB_POSE:
                    ...
                    if (disp)
                    {
                        ...
                        sprintf(buf, "Receive(%ld): TCount=%d Type(POSE)=%d\n %7.2f, %7.2f, %7.2f, %7.2f, %7.2f, %7.2f, %04x, %04x(%s)"
                                , MXTrecv.CCount, MXTrecv.TCount, DispType, p->w.x, p->w.y, p->w.z, p->w.a, p->w.b, p->w.c, p->sflg1, p->sflg2, str);
                        cout << buf << endl;
                    }
                    break;
                case MXT_TYP_PULSE:
